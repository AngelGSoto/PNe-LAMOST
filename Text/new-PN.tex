...
\title[New high-ionization planetary nebula]{First Galactic high-ionization planetary nebula confirmed with LAMOST
  data {\sc alguna sugerencia para el título? como que este no me gusta}}
...
\begin{abstract}
  ...
  color-color diagrams based in GAIA and Pan-STARRS1 photometry........
  The optical LAMOST spectrum of this source show prominent emission lines,
  including highly ionized
  such as He II and [Ar V] lines {\sc Bla Bla Bla Bla}. The LAMOST spectra was
  modelled using the code of photoionization  {\sc cloudy}.  {\sc Bla Bla Bla
  ...
  I found that the mass of the progenitor star is 1.25 M{$\odot$}.
  {\sc Bla Bla Bla Bla
...
The PNe phase begins when gas is ejected from the red giant stars late phase of
...
If this is true, there will be a much larger number of PNe in the Galaxy waiting to be discovered.
...
(e.g. common-envelope phase) are required. \citet{Miszalski:2009} confirms earlier estimates
...
becoming a important task, that every time is more difficult, due to that many of
...
(see \citealp{Kwitter:2022}). Thus, we need to know the number of PNe in
the Galaxy in order to develop accurate models of 
chemical enrichment rates since the Galaxy was young.

Over the years the study of nebulae has been hampered by some problems.
For instance, one of the most important issue is the previous lack of
accurate distances to most Galactic PNe. Fortunately, this problem
tends to be resolved through accurate Gaia CSPN distances.
However, are still too distant and weak for Gaia DR3 and correct
CSPN identification is still a problem \citep{Parker:2022};
Then, these kind of problems could be addressed by increasing the
number of PNe in the Galaxy, because a better study and statistical
could be done. For this reason, several attempted have been performed
...
This manuscript presents a possible new planetary nebula. Based on
its LAMOST spectra, which show strong nebular emission lines,
including high ionization lines, we argue that the object is a
genuine PN. At the time this manuscript was written we noted
that the object is now classified as a galaxy in the \texttt{SIMBAD}
database. However, there are no red-shifted emission lines
in its optical spectrum, discarding its possible extragalactic nature.
This article is organized ad follows: Section~\ref{sec:metho} describes
the observations related to the GAIA,
Pans-STARR and LAMOST. It also presents the new PN find and its
respective optical spectra. Section~\ref{sec:model} describes the modelled {\sc cloudy}
of the new PN, and a comparison of the best fitted model
 with the post-AGB stellar evolution track, and finally
Section~\ref{sec:conclu} discusses the main results and conclusions.

\section{Finding a new possible PN}
\label{sec:metho}

In order to bring to reader a small overview of the data used
...
Note that recently, GAIA have been implemented
to search for central stars of planetary nebulae \citep{Chornay:2020, Gonzalez:2021}.
...
deep archival Pan-STARRS broad-band imagery \citep{Tan:2023}. 
...
Many of broad-band surveys are covering a extensive area of the sky, observing many
astronomical objects with broad-band filters.
It is the case of GAIA and Pans-STARRS1, which
are mapped a large area of the sky.
This means that many of emission line objects could be observed for the broad-band
survey and not for the narrow ones. This means that many emission line objects could
have been observed for broadband surveys and not for narrowband ones. So, we thought that creating a
color-color diagrams based on these surveys to select these class of objects could allow us to find
new emission line objects, even PNe. At the beginning the idea was to identify for new
planetary nebula in GAIA, only.
We started to constructed possibles color-color diagrams to separate PNe from
...
filter as it is possible to see in the Figure~\ref{fig:gaia-ps}. This plot includes PNe
...
from \citet{Akras:2019a}\footnote{We made use of \texttt{TOPCAT} to cross-match each catalog
of objects with GAIA and Pans-STARRS1}.

Figure~\ref{fig:gaia-ps} shows that the many of the PNe have value in
...
Although many of the PNe are located in the  \((G - r)\) color range [-1.0, 8.0]
mixed with other classes of H{$\alpha$} emitters, several PNe are in the region
with \((G - r)\) $\geq$ 1.0 with contamination relatively small. This means that
that objects lactated in that zone have relative high probability to be real PNe.
We visually limited this region with  straight lines as shown in the Figure~\ref{fig:gaia-ps}.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{Figs/color-diagram-ps-gaiaEDR3.pdf}
\caption{Diagram \((G - r)\) versus \((G_{BP} - G_{RP})\) combining the GAIA and Pan-STARRS1 photometry.
  Different emission line objects are presented: PNe from the
  catalog of central star of \citet{Gonzalez:2021} are represented by the dark green symbols,
  orange contours represents the position of the 
  SySt, CVs, SNRs, YSOs, AeBe stars in the diagram and the blue contours represent the
  the positions of stars from the catalog's \citet{Smart:2021}. The continuous lines indicate
  the PN zone that we have chosen in which the contamination due to other emission lines
  objects is small.}
  \label{fig:gaia-ps}
\end{figure}
...
SIMBAD nor listed by \citet{Hou:2016}. To testing the possibility to find
...
\centering
  ...
  \caption{As Figure \ref{fig:gaia-ps} but with the objects classified as
    PN candidate from the list of emission line objects of \citet{Skoda:2020}.
    The LAMOST ID and coordinates are of
    the objects are: J020808.63+491401.0, J090602.63+311037.8, J052527.51+105446.5. } 
  \label{fig:gaia-ps-apply}
\end{figure}

Three objects met this condition as seen in the Figure~\ref{fig:gaia-ps-apply}.
We downloaded the spectra of these objects from the LAMOST database. 
Two of them objects display strong \ha{} emission, but the third one barely shows
emission lines. Of the three only one display the typical emission lines of PNe,
such as [O III], He II together with the Balmer ones.
The other two were discarded to be PNe due to the absence of high-excitation emission lines.

Figure~\ref{fig:spectra} shows the spectrum of the new PN finding in the list of emission
...
its. But note that no emission lines are present in this region of the spectra.
...
\centering
  ...
  \caption{Distribution of the 3,862 Galactic PNe in the HASH database.
    The green and white dots represent the 2,701 labeled \lq\lq True\rq\rq~PNe
    in the HASH catalog.
    While the white dots are the 1161 labeled \lq\lq Likely\rq\rq~and \lq\lq
    Possible\rq\rq~PNe in the same database. The big blue dot is the PNe reported
    in this manuscript.
    Its galactic coordinates are ($l, b$) = (135$^{\circ}$35'11.979", -11$^{\circ}$44'38.994").
    This object is about 2.31 kpc from
    Earth, and 0.47 kpc from the Galactic plane.} 
  \label{fig:position}
\end{figure*}
...
\subsection{Comparing with other high-ionization PNe}
\label{sec:comp}
...
\begin{table}
	...
	\begin{tabular}{lc} % four columns, alignment for each
		...
		Parameter & Value \\
                \hline
		$\log(\mathrm{Temperature~of~central~ionizing~source}) (\mathrm{K})$  & 05.18  \\
		$\log(\mathrm{Luminosity~of~central~ionizing~source) (erg~s^{-1})}$ & 37.22 \\
		$\log(\mathrm{H-density) (cm^{-3})} $ & 03.78  \\
...
they are in calibrate relative flux. This means, it is possible compare it with other
spectra in term of normalize flux. For that, we thought, it is pertinent to
compare the LAMOST spectra of observed PN with models.
...
or less 5$\sigma$ interval. The 1D-Gaussian best fitted model of the observed lines are showed in
Figure~\ref{tab:gausiian-fiited-lines}. Being $\sigma$ the standard deviation of the individual
...
\begin{table*}
	...
	\begin{tabular}{lcccc} % four columns, alignment for each
		...
		\hline
		[Ne III] + H7 3968  & 3967.46 & 0.17 $\pm$ 0.052 &0.16 & 0.021\\
		H{$\delta$} & 4101.74 & 0.24 $\pm$ 0.045 & 0.24& 0.000\\
		H{$\gamma$}  & 4340.71 & 0.43 $\pm$ 0.045 & 0.50 & 2.498 \\
                He II & 4685.99 & 1.05 $\pm$ 0.062 & 1.04 & 0.043\\
                H{$\beta$} & 4861.33& 1.00 $\pm$ 0.056 & 1.00 & 0.000\\
                $[\text{O III}]$ &4958.91 & 0.39 $\pm$ 0.028 & 0.40 & 0.046  \\
                $[\text{O III}]$ & 5006.84& 1.24 $\pm$ 0.063 & 1.19 & 0.542 \\
                He II & 5411.52& 0.08 $\pm$ 0.009 &  0.10 &6.308\\
                H{$\alpha$} & 6562.85& 2.31 $\pm$ 0.098 &  2.78 & 23.234
                \hline
	\end{tabular}
\end{table*}
...
model has value of $\chi^{2} \simeq 32.7$, representing the best fit.
The temperature of the central ionizing source was determined to be  $\sim$150$\times$10$^{4}$K,
...
around 0.02 and 0.00, respectively.    
...
\begin{figure*}
\centering
\includegraphics[width=\linewidth, trim=10 90 10 10, clip]{Figs/model_150000_37.22_3.78.pdf}
...
\subsection{Comparison with post-AGB stellar evolutionary tracks}
\label{sec:tracks}
...
Comparing the model central ionizing black body (T$_{\text{eff}}$ = 150$\times$10$^{3}$K,
...
the first attempt to derivate the physical parameters of the object.
...
\texttt{TOPCAT}\footnote{\url{http://www.star.bristol.ac.uk/~mbt/topcat/}}
...
The 1D-Gaussian fitted to the night emission lines of J020808.63+491401.0 used to measurement the flux lines
and subsequently used to estimate the $\chi^2$ to
find the best-fitted {\sc cloudy} models.
\begin{table*}
\centering
  \caption{The best 1D-Gaussian fitted of the emission lines: [Ne III] 3967.5 \AA, H$\delta$, H$\gamma$, He II 4685.9 \AA, H$\beta$, [O III] 4958.9 \AA,
[O III] 5006.8, He II 5411.7 \AA~and H{$\alpha$} . \label{tab:gausiian-fiited-lines}}\
  \begin{tabular}{l l l }
    ...
    \includegraphics[width=0.3\linewidth, clip]{Figs/Obs_HeII4685.pdf} &
    ...
    \includegraphics[width=0.3\linewidth, clip]{Figs/Obs_HeII5412.pdf} &
