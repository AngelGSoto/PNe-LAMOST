...
than SDSS in the $z$-band \citep{York:2000}. The survey’s five optical
filters, $g$P1, $r$P1, $i$P1, $z$P1, and $y$P1, are described in \citet{Stubbs:2010} and
\citet{Tonry:2012}. At each epoch a single field
...
scientific propose.
Each spectrum is spliced into the blue part and the red part.
...
of stars, for instance CV \citep{Sun:2021}.

\subsection{The find}
\label{sec:find}

Many of broad-band surveys are covering a extensive area of the sky, that
are covered by those narrow-band filters. This means, that many of emission lines
could be observed for the broad-band survey and not for the narrow ones.
Then, we thought that creating color-color diagram to select for these objects
coul give a a ooprtunite for find new emission line objects, in particular PNe.
...
same region in the diagram. Then, I decided to move to Pan-STARRS and combining the two
...
which present objects not reported
...
\begin{table}
	...
	\begin{tabular}{lcccc} % four columns, alignment for each
		...
		Parameter & & Value \\
                \hline
		$\log(T_{\mathrm{BB}}) (\mathrm{K})$  & 5.15 & 5.18 & 5.15\\
		$\log(\mathrm{luminosity) (erg~s^{-1})}$ & 37.15 & 36.98 & 37.25\\
		$\log(\mathrm{Hden) (cm^{-3})} $ & 3.70 & 3.60 & 3.78 \\
                ...
                $\log(\mathrm{Ar/H})$ & & -6.24\\
                 \hline
                 $\chi^2$ & 6.09 &6.16&  6.35 \\
                 $\chi^2_r$ & 1.01 & 1.03&1.06 \\
...
\begin{table*}
	...
	\begin{tabular}{lcccccccc} % four columns, alignment for each
		...
		\hline
		[Ne III] + H7 3968  & 3967.46 & 0.26 $\pm$ 0.068\\
		H{$\delta$} & 4101.74 & 0.29 $\pm$ 0.053\\
		H{$\gamma$}  & 4340.71 & 0.48 $\pm$ 0.047 \\
                He II & 4685.99 & 1.04 $\pm$ 0.061\\
                H{$\beta$} & 4861.33& 1.00 $\pm$ 0.055 \\
                $[\text{O III}]$ &4958.91 & 0.44 $\pm$ 0.031  \\
                $[\text{O III}]$ & 5006.84& 1.21 $\pm$ 0.060 \\
                $[\text{Fe III}]$ & 5412.12& 0.13 $\pm$ 0.016 \\
                H{$\alpha$} & 6562.85& 2.33 $\pm$ 0.097
                \hline
	\end{tabular}
\end{table*}
