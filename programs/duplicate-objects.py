...
dfTrue = pd.read_csv("DR7lamost-True-PN.csv")
...
for i in range(len(coordinates)):
    for j in range(i + 1, len(coordinates)):
        if coordinates[i].separation(coordinates[j]).arcsecond < 1.5:  # Adjust the threshold as needed
...
new_dfTrue.write('DR7lamost-True-PN-duplicate.ecsv', format='ascii.ecsv', overwrite=True)
